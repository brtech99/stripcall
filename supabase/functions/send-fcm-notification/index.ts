...
serve(async (req) => {
  ...
  try {
    ...
    const { title, body, userIds, data, problemId }: NotificationRequest =
      await req.json();
    ...
    if (!body || typeof body !== "string" || body.trim() === "") {
      return new Response(
        JSON.stringify({ error: "Invalid or missing body" }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    for (const id of userIds) {
      if (typeof id !== "string" || !uuidRegex.test(id)) {
        return new Response(
          JSON.stringify({ error: `Invalid user ID format: ${id}` }),
          {
            status: 400,
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          },
    ...
    // Use a service role client to read device tokens (RLS restricts users to their own tokens)
    ...
    const { data: deviceTokens, error: tokenError } = await serviceRoleClient
      .from("device_tokens")
      .select("device_token")
      .in("user_id", userIds);

    console.log("DEBUG: Device tokens query result:", {
      deviceTokens,
      tokenError,
    });
...
async function sendFCMNotification(
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.error(
        `FCM API error for token ${token.substring(0, 20)}...:`,
        responseData,
      );
      return {
        token: token.substring(0, 20) + "...",
