    ...
    if (isCrewMember && crewMemberUser) {
      ...
      if (replySlot) {
        ...
        return await handleCrewMemberReply(
          supabase, crew.id, crewMemberUser, replySlot, messageBody, from
        )
      } else {
        // Broadcast to crew_messages
        return await handleCrewBroadcast(supabase, crew.id, crewMemberUser, messageBody)
      }
    } else {
      // REFEREE/EXTERNAL SMS
      return await handleRefereeMessage(
        supabase, crew.id, crew.event, from, messageBody
      )
    }
...
async function verifyTwilioSignature(req: Request): Promise<boolean> {
  ...
  // Build the validation URL (Twilio sends to this URL)
  const url = req.url
...
async function handleRefereeMessage(
  ...
  // Get reporter name from sms_reporters table
  ...
  const reporterName = reporter?.name || formatPhoneForDisplay(phone)
  ...
  if (existingProblem) {
    ...
    await supabase.from('messages').insert({
      ...
      message: `[SMS from ${reporterName}] ${message}`,
  ...
  console.log(`Created new problem ${newProblem.id} for strip ${strip}, slot +${slot}`)
...
async function handleCrewMemberReply(
  ...
  const senderName = `${user.firstname} ${user.lastname}`.trim() || 'Crew Member'
  ...
  console.log(`Crew member ${senderName} replied to problem ${slotData.problem_id} via slot +${slot}`)

  return twimlResponse(`Message sent to reporter for problem in slot +${slot}.`)
}
