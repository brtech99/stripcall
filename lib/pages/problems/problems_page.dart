...
class _ProblemChatState extends State<ProblemChat> {
  ...
  Future<void> _loadCrewDisplayStyle() async {
    try {
      ...
      if (mounted && crewResponse != null) {
        setState(() {
          _crewDisplayStyle = crewResponse['display_style'] as String?;
        });
      }
    } catch (e) {
      debugPrint('Error loading crew display style: $e');
    }
  }
...
class _ProblemsPageState extends State<ProblemsPage> {
  ...
  int? _selectedCrewId;
  Set<int> _expandedProblems = {}; // Track which problems are expanded
        ...
        for (final problem in newProblems) {
          try {
            if (problem is Map<String, dynamic>) {
              ...
              debugPrint('Unexpected problem data type: ${problem.runtimeType}');
            }
          } catch (e) {
            debugPrint('Error handling new problem: $e');
          }
        }
      }
    } catch (e) {
      debugPrint('Error checking for new problems: $e');
    }
  }
  ...
  Future<void> _checkForResolvedProblems(DateTime since) async {
    ...
    try {
      ...
      if (mounted && resolvedProblems != null && resolvedProblems.isNotEmpty) {
        for (final resolved in resolvedProblems) {
          try {
            ...
            debugPrint('Error handling resolved problem: $e');
          }
        }
      ...
      debugPrint('Error checking for resolved problems: $e');
  ...
  Future<void> _loadCrewInfo() async {
    try {
      ...
      debugPrint('Error loading crew info: $e');
    }
  }
          ...
          for (final json in response) {
            try {
              if (json is Map<String, dynamic>) {
                problems.add(ProblemWithDetails.fromJson(json));
              } else {
                debugPrint('Unexpected problem data type: ${json.runtimeType}');
              }
            } catch (e) {
  ...
  Future<void> _determineUserCrewInfo() async {
    try {
      ...
      debugPrint('Error determining user crew info: $e');
      setState(() {
        _userCrewId = null;
        _userCrewName = null;
      });
  ...
  Future<void> _loadAllCrews() async {
    try {
      ...
      debugPrint('Error loading all crews: $e');
    }
  }
  ...
  Widget _buildExpandedProblem(ProblemWithDetails problem, String status, String? currentUserId) {
    ...
    final crewTypeId = problem.crewTypeId;
    final responders = _responders[problem.id] ?? [];
    final isUserResponding = responders.any((r) => r['user_id'] == currentUserId);
      ...
      children: [
        Row(
          children: [
            ...
            if (!_isReferee && problem.actionString == null) ...[
              if (!isUserResponding)
                ElevatedButton(
        ...
        const SizedBox(height: 4),
        Text(
          'Reported by ${problem.originatorName ?? 'Unknown'} ${_formatTime(startTime)}',
          style: Theme.of(context).textTheme.bodySmall,
        ),
        if (resolvedTime != null) ...[
          const SizedBox(height: 1),
          Text(
            'Resolved by ${problem.actionByName ?? 'Unknown'} ${_formatTime(resolvedTime)}',
            style: Theme.of(context).textTheme.bodySmall,
          ),
        ],
      ],
  ...
  Future<void> _loadMissingOriginatorData(ProblemWithDetails problem) async {
    ...
    try {
      ...
      debugPrint('Error loading originator data: $e');
    }
  }
